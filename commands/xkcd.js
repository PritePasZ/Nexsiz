...
module.exports.run = async (bot, message, args) => {
        ...
        if(!message.channel.nsfw) return message.channel.send(":x: Please run this command in a `NSFW` channel.");
        ...
        let search = args[1] ? `http://xkcd.com/${args[1]}/info.0.json` : "http://xkcd.com/info.0.json";
            try {
                fetch(search).then(res => res.json()).then(res => {
                    if(!res) return message.channel.send(":x: No results found for this comic, sorry!");
